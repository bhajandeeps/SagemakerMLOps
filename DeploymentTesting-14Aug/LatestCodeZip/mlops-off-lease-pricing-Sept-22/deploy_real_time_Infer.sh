#!/bin/bash
: ${ENVIRONMENT:="dev"}
STACK_NAME="mlops-off-lease-pricing-rtInfer-pipeline"
...
ModelEndPointXGBoost=$(aws cloudformation list-exports --query "Exports[?Name==\`mlops-off-lease-pricing-ml-training-pipeline-XGEndpointName\`].Value" --no-paginate --output text)
ModelEndPointLinearLearner=$(aws cloudformation list-exports --query "Exports[?Name==\`mlops-off-lease-pricing-ml-training-pipeline-LREndpointName\`].Value" --no-paginate --output text)
