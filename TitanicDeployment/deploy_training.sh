#!/bin/bash
: ${ENVIRONMENT:="dev"}
STACK_NAME="wi-mlops-titanic-ml-train-piln"
...
RealTimeDataCapture=`cat config/training-config.json | jq -r '.RealTimeDataCapture'`
...
InputS3Prefix=$InputS3Prefix OutputS3Prefix=$OutputS3Prefix RealTimeDataCapture=$RealTimeDataCapture \
...
RandomString=$RandomString ConfigBucket=$ConfigBucket \
XGPreprocessScript=$XGPreprocessScript LRPreprocessScript=$LRPreprocessScript \
PreProcessingContainer=$PreProcessingContainer  --capabilities CAPABILITY_NAMED_IAM 2>&1)
